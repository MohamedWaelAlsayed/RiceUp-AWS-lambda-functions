...
def lambda_handler(event: dict, context):
    ...
    moisture = int(event["moisture"])

    query = "mutation add_reading {createReading(input: {device_id: " + f'"{device_id}"'+ f', timestamp: {timestamp} , temperature: {temperature} , moisture: {moisture} '+ "}) {device_id timestamp temperature moisture createdAt updatedAt _version _deleted _lastChangedAt}}"
    print(query)
    try:
